...
- **Bug reports** — [Open a bug report](https://github.com/ZoomBili/OpenCovibe-web/issues/new?template=bug_report.yml)
- **Feature requests** — [Open a feature request](https://github.com/ZoomBili/OpenCovibe-web/issues/new?template=feature_request.yml)
...
```bash
git clone https://github.com/ZoomBili/OpenCovibe-web.git
cd OpenCovibe-web
npm install
npm run dev
# In another terminal:
...
See [README.md](README.md) for detailed prerequisites (Rust, Node.js, and an existing Agent CLI).
...
- **Comments** — only comment the _why_, not the _what_. See "Comment Standards" in `CLAUDE.md` for detailed rules
...
- Use `RUST_LOG=debug npm run server` for backend debugging
