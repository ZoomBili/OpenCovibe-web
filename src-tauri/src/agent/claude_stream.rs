...
pub(crate) fn resolve_claude_path() -> String {
    ...
    if let Some(custom) = std::env::var("OPENCOVIBE_CLAUDE_PATH")
        .ok()
        .map(|value| value.trim().to_string())
        .filter(|value| !value.is_empty())
    {
        log::debug!(
            "[claude_stream] using claude path from OPENCOVIBE_CLAUDE_PATH: {}",
            custom
        );
...
pub(crate) fn resolve_codex_path() -> String {
    ...
    if let Some(custom) = std::env::var("OPENCOVIBE_CODEX_PATH")
        .ok()
        .map(|value| value.trim().to_string())
        .filter(|value| !value.is_empty())
    {
        log::debug!(
            "[claude_stream] using codex path from OPENCOVIBE_CODEX_PATH: {}",
            custom
        );
        *cached = Some(custom.clone());
        return custom;
