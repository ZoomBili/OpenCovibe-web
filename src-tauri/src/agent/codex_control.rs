...
pub async fn get_codex_models(
    ...
    let codex_binary = resolve_codex_path();
    if (codex_binary.contains('/') || codex_binary.contains('\\'))
        && !std::path::Path::new(&codex_binary).is_file()
    {
    ...
    let mut cmd = tokio::process::Command::new(&codex_binary);
    cmd.arg("app-server")
        .env("PATH", &path_env)
