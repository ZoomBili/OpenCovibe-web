...
pub async fn serve_from_env() -> Result<(), String> {
    use crate::agent::adapter::new_actor_session_map;
    ...
    let bind = std::env::var("OPENCOVIBE_BIND")
        .ok()
        .filter(|value| !value.trim().is_empty())
        .unwrap_or_else(|| "0.0.0.0".to_string());
    if !ALLOWED_BINDS.contains(&bind.as_str()) {
        return Err(format!(
            "OPENCOVIBE_BIND must be one of: {}",
            ALLOWED_BINDS.join(", ")
        ));
    }

    let port = std::env::var("OPENCOVIBE_PORT")
        .ok()
        .map(|value| {
            value
                .parse::<u16>()
                .map_err(|_| "OPENCOVIBE_PORT must be a valid port".to_string())
        })
        .transpose()?
        .unwrap_or(9476);
    if port < 1024 {
        return Err("OPENCOVIBE_PORT must be between 1024 and 65535".to_string());
    }

    let configured_token = std::env::var("OPENCOVIBE_TOKEN")
        .ok()
        .filter(|value| !value.trim().is_empty())
        .or(settings.web_server_token);
    let (token, generated_token) = match configured_token {
    ...
    let environment_origins = std::env::var("OPENCOVIBE_ALLOWED_ORIGINS")
        .ok()
        .map(|value| {
            value
                .split(',')
    ...
    let allowed_origins =
        normalize_origins(&environment_origins.or(settings.web_server_allowed_origins))?;

    let listener = tokio::net::TcpListener::bind((bind.as_str(), port))
        .await
        .map_err(|error| format!("failed to bind {bind}:{port}: {error}"))?;
    let effective_port = Arc::new(AtomicU16::new(port));
    let cancel_token = CancellationToken::new();
    let broadcaster = EventBroadcaster::new();
    let writer = storage::events::global_writer();
    let emitter = Arc::new(BroadcastEmitter::new(writer.clone(), broadcaster.clone()));
    let ws_shutdown = Arc::new(tokio::sync::broadcast::channel::<()>(1).0);
    ...
    let state = AppState {
        ...
        emitter,
        broadcaster,
        token: Arc::new(tokio::sync::RwLock::new(token.clone())),
        token_version: Arc::new(AtomicU64::new(0)),
        ws_shutdown,
        http_sessions: Arc::new(Mutex::new(HashMap::new())),
        effective_port,
        bind_addr: Arc::new(bind.clone()),
        allowed_origins,
    };

    log::info!("OpenCovibe Web listening on http://{bind}:{port}");
    if generated_token {
        log::warn!("Generated one-time access token: {token}");
        log::warn!("Set OPENCOVIBE_TOKEN to keep a stable token across restarts");
    }
    ...
    axum::serve(listener, router)
        ...
        .await
        .map_err(|error| format!("web server error: {error}"))?;
...
async fn shutdown_sessions(state: &AppState) {
    ...
    for mut handle in handles {
        ...
        let _ = handle.cmd_tx.send(ActorCommand::Stop { reply }).await;
        let _ = tokio::time::timeout(std::time::Duration::from_secs(2), response).await;
        if tokio::time::timeout(std::time::Duration::from_secs(2), &mut handle.join_handle)
            .await
            .is_err()
        {
    ...
    let children = {
        let mut process_map = state.process_map.lock().await;
        process_map
            .drain()
            .map(|(_, child)| child)
            .collect::<Vec<_>>()
    ...
    *app.state::<EffectiveWebBind>().0.write().await = bind.to_string();
    ...
    let app_state = AppState {
        ...
        cli_info_cache: app
            ...
            .inner()
            .clone(),
        codex_info_cache: app
            .state::<crate::agent::codex_control::CodexInfoCache>()
            .inner()
            .clone(),
        emitter,
        broadcaster,
