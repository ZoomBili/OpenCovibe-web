...
pub async fn dispatch_command(
    ...
    let result = match method {
        ...
        "get_server_home" => Ok(json!(crate::storage::home_dir())),
        ...
        "get_codex_models" => {
            let force = params
                ...
                .unwrap_or(false);
            match crate::agent::codex_control::get_codex_models(&state.codex_info_cache, force)
                .await
            {
                Ok(models) => serde_json::to_value(models).map_err(|error| error.to_string()),
                Err(error) => {
                    log::warn!(
                        "[dispatch] Codex model discovery failed ({}): {}, using fallback",
                        error.code,
                        error.message
                    );
                    serde_json::to_value(crate::agent::codex_control::fallback_models())
