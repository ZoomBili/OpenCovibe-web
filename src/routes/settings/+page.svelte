<script lang="ts">
  ...
  async function applyZoomQueued(factor: number) {
    ...
    zoomFlying = true;
    try {
      const { applyBrowserZoom } = await import("$lib/utils/browser");
  ...
  onMount(async () => {
    ...
    api
      .getServerHome()
      .then(async (home) => {
        if (!home) throw new Error("Server home unavailable");
        const parts = splitPath(home.replace(/[/\\]+$/, ""));
        currentUsername = parts[parts.length - 1] || "";
        const separator = home.includes("\\") ? "\\" : "/";
        const absPath = `${home.replace(/[/\\]+$/, "")}${separator}.claude${separator}keybindings.json`;
      ...
      <Card class="p-6 space-y-5">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-xs text-muted-foreground mt-0.5">
              ...
              <code class="text-xs">RUST_LOG=debug npm run server</code>
            </p>
          </div>
          <button
        ...
        {#if debugOn}
          ...
          <div class="rounded-md bg-muted/50 p-3">
            ...
            <div class="flex items-center gap-2">
              <code class="flex-1 text-xs font-mono break-all">RUST_LOG=debug npm run server</code>
              <button
                class="shrink-0 rounded border px-2 py-1 text-[10px] transition-colors hover:bg-accent"
                onclick={async () => {
                  await navigator.clipboard.writeText("RUST_LOG=debug npm run server");
                  rustCmdCopied = true;
